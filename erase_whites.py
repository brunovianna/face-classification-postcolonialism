...
def bleach(img_path, df):
    """Write white rectangles on white people
    """
    img = cv2.imread(img_path)
    # img  = cv2.cvtColor(color, cv2.COLOR_BGR2RGB)
    for row in df.iterrows():
        ...
        if race == 'White':
            ...
            rect_top = int(math.floor (center_y / 102) * 102) -1
            rect_bottom = int((math.floor (center_y / 102) +1 )* 102) -1
            rect_left = int(math.floor (center_x / 102) * 102)
            rect_right = int((math.floor (center_x / 102) +1)* 102)
            cv2.rectangle(img, (rect_left, rect_top), (rect_right, rect_bottom), (255, 255, 255), -1)
            #text_showed = "{} {} {} {}".format(rect_left,rect_top,rect_right,rect_bottom)
            #font = cv2.FONT_HERSHEY_DUPLEX
            #img_width = img.shape[1]
            #cv2.putText(img, text_showed, (left + 6, bottom - 6), font, 0.5, (255, 255, 255), 1)
    return img
